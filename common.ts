...
import { readableStreamToString, FrontCoreError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new FrontCoreError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new FrontCoreError(e, e.response?.data)
                }
                if (innerError instanceof FrontCoreError) {
                    // Got "FrontCoreError" from the above try block
                    throw innerError;
                }
